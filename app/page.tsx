...
// Import Components
import { ColumnItem, ColumnLayout, Divider } from 'thread-ui';
...
const places: ColumnItem[] = placeItems.map((item) => ({
	title: item.title,
	description: item.description,
	image: <Image src={item.image.src} alt={item.image.alt} />,
}));
...
const movies: ColumnItem[] = movieItems.map((item) => ({
	image: <Image src={item.image.src} alt={item.image.alt} />,
}));
...
export default function Home() {
	return (
		<main>
			...
			<ColumnLayout mdcol={2} lgcol={4} items={places} />
			<ColumnLayout
				title="My Favorite Movies"
				caption="These movies are all a bit different, but each represents a part of me."
				mdcol={1}
				lgcol={3}
				items={movies}
			/>
